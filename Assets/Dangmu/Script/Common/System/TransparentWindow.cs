...
public class TransparentWindow : MonoBehaviour
{
	...
	static public void SetWindowsMouseAble()
	{
		#if !UNITY_EDITOR
		// Get a handle to the window
		var hwnd = GetActiveWindow();
		...
		SetWindowPos(hwnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_SHOWWINDOW | SWP_NOSIZE | SWP_NOMOVE);
		#endif
	}

	//for dangmu...
	static public void SetWindowsMouseDisable()
	{
		#if !UNITY_EDITOR
		// Get a handle to the window
		var hwnd = GetActiveWindow();
	...
	void StartBak()
	{
		...
		// https://www.cnblogs.com/linyawen/archive/2011/01/17/1937863.html
		uint intExTemp = GetWindowLong(hwnd, GWL_EXSTYLE);
		SetWindowLong(hwnd, GWL_EXSTYLE, intExTemp | WS_EX_TRANSPARENT | WS_EX_LAYERED); //WS_EX_TOPMOST | WS_POPUP |  WS_VISIBLE 
		//SetWindowLong(hwnd, GWL_EXSTYLE, GetWindowLong(hwnd, GWL_EXSTYLE) & ~WS_BORDER & ~WS_CAPTION);
		...
		*/
		...
		//窗口置顶必须的...
		SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_ASYNCWINDOWPOS | SWP_NOMOVE | SWP_NOSIZE); //| SWP_NOSIZE..
		//SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_SHOWWINDOW); //| SWP_NOSIZE..
	...
	/*
	public void SetSecondWindowTopMost() {
		var hwnd = MultiScreenWndProc.GetSecondWindowHandle ();
		...
		//SetWindowLong (hwnd, -20, (uint)524288 | (uint)32);//GWL_EXSTYLE=-20; WS_EX_LAYERED=524288=&h80000, WS_EX_TRANSPARENT=32=0x00000020L
		//SetLayeredWindowAttributes (hwnd, 0, 255, 2);// Transparency=51=20%, LWA_ALPHA=2
		SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, fWidth, fHeight, SWP_FRAMECHANGED | SWP_NOSIZE | SWP_NOMOVE); //SWP_FRAMECHANGED = 0x0020 (32); //SWP_SHOWWINDOW = 0x0040 (64)
		#endif
	}*/
}